...
import { WAREHOUSES, ALERTS, CONSOLIDATED } from '@/data/mock';
...
export default function DashboardPage() {
  const { t } = useLanguage();
  const { totalLots, enTransit, distribution } = CONSOLIDATED;
  const exceptions = warehouseExceptions(WAREHOUSES);
  const alerts = sortAlertsBySeverity(ALERTS);
  const criticalCount = ALERTS.filter((a) => a.severity === 'critique').length;
  const shownAlerts = alerts.slice(0, ALERT_PREVIEW);
  const moreAlerts = alerts.length - shownAlerts.length;
        ...
        <div className="flex items-center justify-between mb-[18px]">
          <div className="flex items-center gap-3">
            ...
            <span className="inline-flex items-center rounded-full bg-[#FEF2F2] border border-[#FCA5A5] px-[10px] py-[3px] text-[11px] font-semibold text-[#9B1C1C]">
              {alerts.length} {t('alerts_label')}{criticalCount > 0 ? ` · ${criticalCount} ${t('critical_label')}` : ''}
            </span>
          </div>
